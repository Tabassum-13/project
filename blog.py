...
def save_to_cache(key, value, ttl=3600):
    ...
    c.execute('REPLACE INTO cache (key, value, timestamp) VALUES (?, ?, ?)', (key, value, timestamp + ttl))
    conn.commit()
    conn.close()
...
def fetch_news(api_url, retries=3, backoff_factor=1.5):
    ...
    for attempt in range(retries):
        try:
            ...
            save_to_cache(api_url, response.text)
...
# Streamlit app
st.subheader('Recent News Articles')
api_key = '50aec2557c344e3bb301a144b1e673dd'  # Replace with your API key
api_url = f'https://newsapi.org/v2/top-headlines?country=in&apiKey={api_key}'
